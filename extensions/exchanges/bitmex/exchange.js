...
//TODO Implement process start of Bitmex Delta server.
// We need to spawn Bitmex delta server for this to work properly.
// And use it for our api calls.
...
module.exports = function bitmex (conf) {
  ...
  /**
   * ORIGINAL CODE, using it to slowly incorporate functions of Bitmex
   *
  ...
  function publicClient () {
    //TODO: Without key, you can read tickers at Bitmex, setup here.
    if (!public_client) public_client = new BFX(null,null, {version: 2, transform: true}).rest
    return public_client
  }
  ...
  function wsClient () {
    if (!ws_client) {
      ...
      //ws_client = new BFX(conf.bitmex.key, conf.bitmex.secret, {version: 2, transform: true}).ws
      //TODO: Establish and verify connection to Delta server.
